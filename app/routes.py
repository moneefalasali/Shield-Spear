...
def join_coop_session(session_code):
    ...
    if request.method == 'POST':
        return jsonify({'success': True, 'redirect_url': url_for('challenges.play_coop', session_code=session_code)})
    
    return redirect(url_for('challenges.play_coop', session_code=session_code))

@challenges_bp.route('/coop/play/<session_code>')
@login_required
def play_coop(session_code):
